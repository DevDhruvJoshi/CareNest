...
function shouldExclude(relPath) {
  ...
  const excludedTop = new Set([
    ...
    '.idea', '.vscode', '.DS_Store', 'Thumbs.db', '.cache', 'tmp', 'temp'
  ]);
...
function syncOnce({ silent = false } = {}) {
  ...
  for (const e of mirrorEntries) {
    if (e === '.git') continue;
    fs.rmSync(path.join(mirrorDir, e), { recursive: true, force: true });
  }
...
function daemonLoop(intervalMs) {
  const tick = () => { try { syncOnce({ silent: true }); } catch {} };
  tick();
...
function main() {
  ...
  if (watch) {
    try {
      ...
      const ignored = (p) => shouldExclude(path.relative(root, p));
      const watcher = chokidar.watch(root, { ignored, ignoreInitial: true, persistent: true, depth: 99, awaitWriteFinish: { stabilityThreshold: 1000, pollInterval: 200 } });
      let timer = null;
      const trigger = () => { clearTimeout(timer); timer = setTimeout(() => { try { syncOnce({ silent: true }); } catch {} }, 5000); };
